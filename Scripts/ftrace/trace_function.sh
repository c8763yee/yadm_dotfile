#!/usr/bin/env bash
# Usage: $0 [-f <func>...] [-n <notrace_func>...] -- <command> [args...]

TRACEFS=/sys/kernel/tracing

die()   { echo "$0: error: $1" >&2; exit 1; }
usage() { echo "Usage: $0 [-f <func>...] [-n <notrace_func>...] -- <command> [args...]" >&2; exit 1; }
...
if [[ ${#funcs[@]} -gt 0 ]]; then
    for func in "${funcs[@]}"; do
        echo "$func" >> "$TRACEFS/set_ftrace_filter" || die "unknown function: $func"
    done
fi

for func in "${notrace_funcs[@]}"; do
    echo "$func" >> "$TRACEFS/set_ftrace_notrace"
done
